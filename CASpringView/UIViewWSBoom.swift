...
//            opacityAnimation.delegate = self
    ...
    //随机产生缩放数值
    fileprivate func makeScaleValue() -> Float{
        
        return 1 - 0.7 * Float (Int(arc4random() % 101) - 50) / 50
    }
...
//            let bitmapData = UnsafeMutableRawPointer.allocate(capacity: bitmapByteCount)
    ...
    func getPixelColorAtLocation(_ point:CGPoint) -> UIColor{
        ...
        let resData : UnsafePointer<UInt8> = CFDataGetBytePtr(self.cgImage?.dataProvider?.data)
